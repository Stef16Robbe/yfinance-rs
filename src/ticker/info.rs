...
pub(super) async fn fetch_info(
    ...
    let (quote, profile, price_target, rec_summary, esg_summary, calendar) =
        Box::pin(fetch_info_parts(client, symbol, cache_mode, retry_override)).await?;
    Ok(Info {
        snapshot: quote.to_snapshot(),
        key_statistics: quote.to_key_statistics(),
        profile,
...
> {
    ...
    let (quote_res, profile_res, price_target_res, rec_summary_res, esg_res, calendar_res) = tokio::join!(
